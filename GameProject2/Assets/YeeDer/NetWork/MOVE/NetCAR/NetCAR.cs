...
public class NetCAR : MonoBehaviour {
    ...
    [SerializeField]
    public List<WheelCollider> m_WheelColliders;
    [SerializeField]
    ...
    private void Start()
    {
        int connectID = transform.parent.GetComponent<RobotProperty>().connectID;
    ...
    public void getWheel()
    {
        ...
        for (int i = 0; i < wheel.Length; i++)
            if (wheel[i].GetComponent<NetWheel>())
                m_WheelColliders.Add(wheel[i]);

        GetComponent<Rigidbody>().ResetCenterOfMass();
        GetComponent<FixedJoint>().connectedBody = connectBody;
    }
    ...
    {
        for (int i = 0; i < m_WheelColliders.Count; i++)//讓mesh轉動同步
        {
            Quaternion quat;
            Vector3 position;
            m_WheelColliders[i].GetWorldPose(out position, out quat);
            GameObject wheelmesh = m_WheelColliders[i].GetComponentInChildren<Renderer>().gameObject;

            //wheelmesh.transform.Rotate(m_WheelColliders[i].rpm, 0, 0);
            Vector3 r = wheelmesh.transform.eulerAngles;
            r.x = quat.eulerAngles.x;
            r.y = quat.eulerAngles.y;
            wheelmesh.transform.eulerAngles = r;
        }
        ...
        {
            if (m_WheelColliders[i].transform.GetChild(0).name == "RotateWheel")
            {
        ...
        for (int i = 0; i < m_WheelColliders.Count; i++)
        {
            ...
            r.y = 0;
            r.z = 0;
            wheelmesh.transform.localEulerAngles = r;
        }
    }
    ...
    private void ApplyDrive(float accel, float footbrake)//前進的關鍵
    {
        ...
        for (int i = 0; i < m_WheelColliders.Count; i++)//煞車
        {
