...
public class CAR : MonoBehaviour
{
    ...
    [SerializeField]
    private List<WheelCollider> m_WheelColliders;
    [SerializeField]
    ...
    void getWheel()
    {
        ...
        m_WheelColliders.AddRange(wheel);
    }
    ...
    public void Move(float steering, float accel, float footbrake, float handbrake)
    {
        for (int i = 0; i < 4; i++)
        {
            ...
            GameObject wheelmesh = m_WheelColliders[i].transform.GetChild(0).gameObject;
        ...
        for (int i = 0; i < m_WheelColliders.Count; i++)
            if (m_WheelColliders[i].name == "RotateWheel" || m_WheelColliders[i].name == "RotateWheel")
            {
        ...
        if (handbrake > 0f)
        {
            ...
            for (int i = 0; i < m_WheelColliders.Count; i++)
                    m_WheelColliders[i].brakeTorque = hbTorque;//煞車
        }
    ...
    private void ApplyDrive(float accel, float footbrake)//前進的關鍵
    {
        ...
        switch (m_CarDriveType)
        {
            case CarDriveType.FourWheelDrive:
                ...
                for (int i = 0; i < 4; i++)
                {
                    m_WheelColliders[i].motorTorque = thrustTorque* speedMultiple;
                }
                break;
