...
namespace Prototype.NetworkLobby
{
    public class LobbyManager : NetworkLobbyManager 
    {
		...
		public string[] playScenes={"stadium","waterfront"};
		...
		{
			Debug.Log ("now" + conn.connectionId);
			//gamePlayerPrefab.GetComponent<NetPlayerController>().connectID = conn.connectionId;
			var player = (GameObject)GameObject.Instantiate(gamePlayerPrefab, new Vector3(0, 5, 0), Quaternion.identity);
            player.GetComponent<NetPlayerController>().connectID = conn.connectionId;
       ...
       /* public override void OnServerAddPlayer(NetworkConnection conn, short playerControllerId)//自訂義加入腳色
        ...
        public IEnumerator ServerCountdownCoroutine()
        {
            ...
            for (int i = 0; i < lobbySlots.Length; ++i)
            {
                if (lobbySlots[i] != null)
                {
					...
					(lobbySlots [i] as LobbyPlayer).RpcSetBackground (false);
                }
            }
			playScene = playScenes [mode];
		...
		public void OnClientChangeTeam(string name)
		{
			for (int i = 0; i < lobbySlots.Length; ++i)
			{
				if (lobbySlots[i] != null)
				{
					(lobbySlots[i] as LobbyPlayer).RpcPlayerChangeTeam(name);
				}
			}

		}
		public void OnModeChanged(int newValue)
		{
			mode = newValue;
			for (int i = 0; i < lobbySlots.Length; ++i)
			{
				if (lobbySlots[i] != null)
				{
					(lobbySlots[i] as LobbyPlayer).RpcModeChanged(newValue);
				}
			}
		}
